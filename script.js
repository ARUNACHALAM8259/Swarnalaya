...
// ─────────────────────────────────────────────────────────────────
// initProfileDropdown()
// Call once per page. Reads localStorage for auth state and:
//  - Shows avatar circle with user initial when logged in
//  - Shows "Login/Signup" when logged out
//  - Switches dropdown items based on auth state
//  - Wires up the logout item to redirect to logout.html
// ─────────────────────────────────────────────────────────────────
function initProfileDropdown() {
  const menu = document.querySelector(".user-profile-menu");
  if (!menu) return;

  const avatarImg = menu.querySelector(".user-avatar-btn");
  const dropdown = menu.querySelector(".profile-dropdown-menu");
  if (!dropdown) return;

  // Read session
  let user = null;
  try {
    const raw = localStorage.getItem("swarnalaya_auth_user");
    if (raw) user = JSON.parse(raw);
  } catch (_) {}

  const isLoggedIn = !!user;

  // ── Build / inject the dropdown contents ─────────────────
  // Clear existing items and rebuild so this works on all pages
  dropdown.innerHTML = "";

  if (isLoggedIn) {
    // ── Logged-in: show user header + Dashboard + Logout ──
    const name = user.name || "";
    const phone = user.phone || user.mobile || "";
    const email = user.email || "";

    // Display name
    let displayName = name || email || phone || "My Account";
    if (displayName.endsWith("@swarnalaya.com")) {
    ...
    let phoneLine = "";
    if (phone) {
      const cleaned = phone.toString().replace(/^91/, "");
      phoneLine = "+91 " + cleaned.replace(/(\d{5})(\d{5})/, "$1 $2");
    } else if (email) {
      phoneLine = email;
    }
    ...
    // Swap avatar image → circle
    if (avatarImg) avatarImg.style.display = "none";

    // Create avatar circle if not already there
    let circle = menu.querySelector(".user-avatar-circle");
    if (!circle) {
      circle = document.createElement("div");
      circle.className = "user-avatar-circle visible";
      menu.insertBefore(circle, dropdown);
    } else {
    ...
    header.className = "dropdown-user-header visible";
    header.innerHTML = `
      <div class="dropdown-user-name">${displayName}</div>
      ${phoneLine ? `<div class="dropdown-user-phone">${phoneLine}</div>` : ""}
    `;
    ...
    dashItem.className = "dropdown-item";
    dashItem.innerHTML = '<i class="fas fa-tachometer-alt"></i> My Dashboard';
    dropdown.appendChild(dashItem);

    // My Orders (placeholder)
    const ordersItem = document.createElement("a");
    ordersItem.href = "dashboard.html";
    ordersItem.className = "dropdown-item";
    ordersItem.innerHTML = '<i class="fas fa-box"></i> My Orders';
    ...
    contactItem.innerHTML = '<i class="fas fa-headset"></i> Contact Us';
    dropdown.appendChild(contactItem);

    // Logout
    const logoutItem = document.createElement("button");
    logoutItem.className = "dropdown-item logout-item";
    logoutItem.innerHTML = '<i class="fas fa-sign-out-alt"></i> Sign Out';
    logoutItem.addEventListener("click", () => {
      localStorage.removeItem("swarnalaya_auth_user");
      localStorage.removeItem("swarnalaya_cart");
      localStorage.removeItem("swarnalaya_wishlist");
      ...
      try {
        fetch("./api/activity", {
          ...
          body: JSON.stringify({
            activityType: "logout",
            details: "User signed out directly from header menu",
    ...
    // ── Logged-out: show avatar image + Login/Signup ──────
    if (avatarImg) avatarImg.style.display = "";
    const circle = menu.querySelector(".user-avatar-circle");
    if (circle) circle.style.display = "none";

    const loginItem = document.createElement("a");
    loginItem.href = "login.html";
    loginItem.className = "dropdown-item";
    loginItem.id = "loginSignupBtn";
    loginItem.innerHTML = '<i class="fas fa-sign-in-alt"></i> Login / Sign Up';
  ...
  const trigger = menu.querySelector(".user-avatar-circle") || avatarImg;
  if (trigger) {
  ...
  // Also allow clicking the avatar image (logged-out state)
  if (avatarImg) {
...
// Auto-init on DOMContentLoaded for all pages that include script.js
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function performSearch(query) {
    ...
    productCards.forEach((card) => {
      ...
      const details = card.querySelector(".scheme-details")
        ? card.querySelector(".scheme-details").textContent.toLowerCase()
        : "";

      if (
        title.includes(term) ||
        title.includes(termSingular) ||
        category.includes(term) ||
        category.includes(termSingular) ||
        desc.includes(term) ||
        desc.includes(termSingular) ||
        details.includes(term) ||
        details.includes(termSingular)
      ) {
    ...
    arrivalCards.forEach((card) => {
      ...
      if (
        ...
        imgAlt.includes(termSingular) ||
        imgSrc.includes(term) ||
        imgSrc.includes(termSingular)
      ) {
        card.style.display = "block";
        card.style.opacity = "1";
        card.style.transform = "scale(1)";
  ...
  function openCameraModal() {
    ...
    if (!overlay) {
      ...
      document
        .getElementById("cameraShutterBtn")
    ...
    navigator.mediaDevices
      ...
      .catch((err) => {
        ...
        `;
        document
          .getElementById("cameraCloseErrorBtn")
          .addEventListener("click", closeCameraModal);
      });
  }
  ...
  function capturePhoto() {
    ...
    document
      .getElementById("cameraRetakeBtn")
      .addEventListener("click", openCameraModal);
    document
      .getElementById("cameraUseBtn")
      .addEventListener("click", function () {
        ...
        closeCameraModal();
        alert(
          "Simulating Visual Search: Analyzing design patterns from captured photo to find matching jewelry collections.",
        );
      });
  }
  ...
  micBtn.addEventListener("click", function (e) {
    ...
    if (!SpeechRecognition) {
      alert(
        "Speech Recognition is not supported by your current browser. Please try Google Chrome or Safari.",
      );
      return;
    }
  ...
  function openVoiceSearchModal() {
    ...
    if (!overlay) {
      ...
      document.body.appendChild(overlay);
      document
        .getElementById("micCloseBtn")
        .addEventListener("click", closeVoiceSearchModal);
    }
    ...
    recognition.onerror = function (event) {
      ...
      if (event.error === "not-allowed") {
        hintDiv.innerHTML =
          '<span class="mic-error-msg">Microphone permission denied.<br>Please allow access in your settings.</span>';
      } else {
  ...
  function setupMegaDropdown(btnId, dropdownId) {
    ...
    function switchTab(targetItem) {
      ...
      const targetPanel = dropdown.querySelector(
        `#tab-${btnId.replace("MegaMenuBtn", "")}-${tabName}`,
      );
      if (targetPanel) {
  ...
  document.addEventListener("click", function (e) {
    ...
    const alljewelleryDropdown = document.getElementById(
      "alljewelleryMegaDropdown",
    );
    ...
    if (
      alljewelleryDropdown &&
      alljewelleryDropdown.classList.contains("show") &&
      !alljewelleryDropdown.contains(e.target) &&
      e.target !== alljewelleryBtn
    ) {
    ...
    if (
      goldDropdown &&
      goldDropdown.classList.contains("show") &&
      !goldDropdown.contains(e.target) &&
      e.target !== goldBtn
    ) {
    ...
    if (
      diamondDropdown &&
      diamondDropdown.classList.contains("show") &&
      !diamondDropdown.contains(e.target) &&
      e.target !== diamondBtn
    ) {
    ...
    if (
      earringsDropdown &&
      earringsDropdown.classList.contains("show") &&
      !earringsDropdown.contains(e.target) &&
      e.target !== earringsBtn
    ) {
    ...
    if (
      ringsDropdown &&
      ringsDropdown.classList.contains("show") &&
      !ringsDropdown.contains(e.target) &&
      e.target !== ringsBtn
    ) {
    ...
    if (
      dailywearDropdown &&
      dailywearDropdown.classList.contains("show") &&
      !dailywearDropdown.contains(e.target) &&
      e.target !== dailywearBtn
    ) {
    ...
    if (
      gemstoneDropdown &&
      gemstoneDropdown.classList.contains("show") &&
      !gemstoneDropdown.contains(e.target) &&
      e.target !== gemstoneBtn
    ) {
    ...
    if (
      weddingDropdown &&
      weddingDropdown.classList.contains("show") &&
      !weddingDropdown.contains(e.target) &&
      e.target !== weddingBtn
    ) {
...
window.shiftStories = function (dir) {
  const cards = document.querySelectorAll(".story-card");
  if (!cards.length) return;
  window.activeStoryIndex =
    (window.activeStoryIndex + dir + cards.length) % cards.length;
  window.updateStories();
};
...
(function () {
  ...
  function updateTopTicker(data) {
    ...
    const content = `
        ...
        ${formatTopItem("Silver (91.6)", data.silver.price, data.silver.change, data.silver.direction)}
  ...
  function updateNavbarTickerItems(data) {
    ...
    const arrow24 =
      data.gold24K.direction === "up"
        ? " ▲"
        : data.gold24K.direction === "down"
          ? " ▼"
          : "";
    const arrow22 =
      data.gold22K.direction === "up"
        ? " ▲"
        : data.gold22K.direction === "down"
          ? " ▼"
          : "";
    const arrow18 =
      data.gold18K.direction === "up"
        ? " ▲"
        : data.gold18K.direction === "down"
          ? " ▼"
          : "";
    const arrowSil =
      data.silver.direction === "up"
        ? " ▲"
        : data.silver.direction === "down"
          ? " ▼"
          : "";
    ...
    rateItems = [
      {
        label: "Gold 91.6 (22K)",
        value: `₹${data.gold22K.price.toLocaleString("en-IN")}/g${arrow22}`,
      },
      {
        label: "Gold 100 (24K)",
        value: `₹${data.gold24K.price.toLocaleString("en-IN")}/g${arrow24}`,
      },
      {
        label: "Gold 75 (18K)",
        value: `₹${data.gold18K.price.toLocaleString("en-IN")}/g${arrow18}`,
      },
      {
        label: "Silver (91.6)",
        value: `₹${data.silver.price.toFixed(2)}/g${arrowSil}`,
      },
    ];
  ...
  function updateHomepageRatesCard(data) {
    ...
    if (cardLastUpdated) {
      const timeText = data.updatetimeText
        ? data.updatetimeText
    ...
    fetch("./api/rates")
      ...
      .catch((err) => {
        ...
        const fallbackData = {
          ...
          gold22K: { price: 14161, change: -0.2, direction: "down" },
...
(function () {
  ...
  const i18n = {
    en: {
      ...
      welcome:
        "Hello! Welcome to Swarnalaya Jewellers. I am your personal shopping assistant. How can I help you today?",
      enterName: "To book a showroom appointment, please enter your full name:",
      enterPhone:
        "Thank you, {name}. Please enter your 10-digit mobile number:",
      enterDate: "Great! Please select or enter your preferred visit date:",
      enterTime:
        "Almost done. Please select or enter your preferred time slot:",
      ...
      invalidDate:
        "Please enter a valid date (e.g. DD-MM-YYYY or select from calendar):",
      ratesFetchError:
        "Unable to retrieve today's rates. Please try again later.",
      whatsappBtn: "Chat on WhatsApp",
      selectBudget:
        "Please select your budget range to see product recommendations:",
    ...
    ta: {
      ...
      welcome:
        "வணக்கம்! ஸ்வர்ணாலயா ஜுவல்லர்ஸிற்கு உங்களை வரவேற்கிறோம். நான் உங்கள் தனிப்பட்ட உதவி உதவியாளர். இன்று உங்களுக்கு நான் எவ்வாறு உதவ முடியும்?",
      enterName:
        "ஷோரூம் அப்பாயிண்ட்மெண்ட் பதிவு செய்ய, தயவுசெய்து உங்கள் முழுப் பெயரை உள்ளிடவும்:",
      enterPhone: "நன்றி, {name}. உங்கள் 10-இலக்க மொபைல் எண்ணை உள்ளிடவும்:",
      enterDate:
        "அருமை! நீங்கள் வர விரும்பும் தேதியைத் தேர்ந்தெடுக்கவும் அல்லது உள்ளிடவும்:",
      enterTime:
        "முடிவடையும் நிலை. நீங்கள் வர விரும்பும் நேரத்தைத் தேர்ந்தெடுக்கவும் அல்லது உள்ளிடவும்:",
      appointmentConfirmed:
        "### அப்பாயிண்ட்மெண்ட் உறுதிசெய்யப்பட்டது! 📅\n\nநன்றி, **{name}**! ஸ்வர்ணாலயா விழுப்புரம் ஷோரூமில் உங்களுக்கான அப்பாயிண்ட்மெண்ட் பதிவு செய்யப்பட்டுள்ளது:\n- **தேதி:** {date}\n- **நேரம்:** {time}\n- **தொலைபேசி:** {phone}\n\nஉங்கள் வருகையை உறுதிப்படுத்த எங்கள் வாடிக்கையாளர் பிரதிநிதி விரைவில் உங்களை அழைப்பார். நன்றி!",
      invalidPhone: "தயவுசெய்து சரியான 10-இலக்க மொபைல் எண்ணை உள்ளிடவும்:",
      invalidDate:
        "தயவுசெய்து சரியான தேதியை உள்ளிடவும் (DD-MM-YYYY அல்லது காலெண்டரிலிருந்து தேர்ந்தெடுக்கவும்):",
      ratesFetchError:
        "இன்றைய விலையைப் பெற முடியவில்லை. பின்னர் மீண்டும் முயற்சிக்கவும்.",
      whatsappBtn: "வாட்ஸ்அப்பில் அரட்டையடிக்க",
      selectBudget:
        "தயாரிப்பு பரிந்துரைகளைக் காண உங்கள் பட்ஜெட் வரம்பைத் தேர்ந்தெடுக்கவும்:",
  ...
  function setupEventListeners() {
    ...
    langToggle.addEventListener("click", () => {
      ...
      document.getElementById("chatbotHeaderTitleText").textContent =
        i18n[chatState.lang].headerTitle;
      document.getElementById("chatbotHeaderStatusText").textContent =
        i18n[chatState.lang].headerStatus;
      ...
      if (bubbleText)
        bubbleText.textContent = i18n[chatState.lang].speechBubble;
  ...
  function addBotMessage(text, isHTML = false) {
    ...
    if (isHTML) {
      ...
      let formatted = text
        ...
        .replace(
          /📞/g,
          '<i class="fas fa-phone" style="color: #c5a059; margin-left: 3px; margin-right: 3px;"></i>',
        )
        .replace(/\n/g, "<br/>");
  ...
  async function processChatMessage(text) {
    ...
    try {
      ...
      console.warn(
        "Unable to fetch reply from Gemini, using offline fallback.",
        err,
      );
  ...
  function getOfflineFallbackClient(message) {
    ...
    if (currentLang === "ta") {
      if (
        msg.includes("விழுப்புரம்") ||
        msg.includes("முகவரி") ||
        msg.includes("தொடர்பு") ||
        msg.includes("போன்") ||
        msg.includes("வாட்ஸ்அப்")
      ) {
      ...
      if (
        msg.includes("திட்டம்") ||
        msg.includes("விருத்தி") ||
        msg.includes("சேமிப்பு")
      ) {
  ...
  function showCollectionsAction() {
    ...
    container.querySelectorAll(".bot-rich-item").forEach((item) => {
      item.addEventListener("click", () => {
        ...
        showTypingIndicator();
        setTimeout(() => {
          const title = i18n[chatState.lang].recommendationsTitle.replace(
            "{budget}",
            budgetName,
          );
  ...
  function showContactAction() {
    ...
    const link =
      "https://wa.me/919837371616?text=Hi%20Swarnalaya%2C%20I%20have%20a%20query%20about%20your%20jewellery...";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (container) {
    ...
    if (!modal) {
      switchTryOnCategory(
        "necklaces",
        document.querySelector(".tryon-tab-btn"),
      );
    }
  }
});
...
function populateTryOnProducts() {
  ...
  list.forEach((prod) => {
    ...
    item.className =
      "tryon-grid-item" +
      (prod.src === tryOnState.currentItem ? " active" : "");
  ...
  if (tryOnState.isCameraOn) {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        if (camBtn) {
          camBtn.innerHTML =
            '<i class="fas fa-camera-retro"></i> Stop Live Camera';
...
window.filterExplorerCatalog = function (itemName, categoryName, event) {
  ...
  const matchedProd =
    products.find(
      (p) =>
        p.name.toUpperCase().includes(itemName.toUpperCase()) ||
  ...
  if (matchedProd) {
    ...
    showExplorerToast(
      `Selected ${matchedProd.name} from ${itemName} collection!`,
    );
  } else {
...
function showExplorerToast(message) {
  ...
  if (!container) {
    ...
    container.style.cssText =
      "position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%); z-index: 9999; display: flex; flex-direction: column; gap: 10px; pointer-events: none;";
    document.body.appendChild(container);
  }

  const toast = document.createElement("div");
  toast.style.cssText =
    "background: rgba(26, 95, 74, 0.95); color: #fff; border: 1px solid #c5a059; padding: 12px 24px; border-radius: 30px; font-weight: 600; font-size: 14px; box-shadow: 0 10px 25px rgba(0,0,0,0.25); backdrop-filter: blur(8px); transform: translateY(50px); opacity: 0; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); text-align: center;";
...
window.logCustomerActivity = function (activityType, details = "") {
  try {
    ...
    fetch("./api/activity", {
      ...
      .then((res) => {
        if (!res.ok)
          console.warn("Activity tracking warning: HTTP status", res.status);
      })
      .catch((err) => {
